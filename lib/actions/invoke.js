'use strict';
const { messages } = require('elasticio-node');
const aws = require('aws-sdk');
const debug = require('debug')('aws-lambda-component:invoke');
const jsonata = require('jsonata');


/**
* This function will be called by the platform to verify given credentials
*
* @returns Promise
**/
function processAction(msg, cfg) {
    const lambda = new aws.Lambda({
        region: cfg.region,
        accessKeyId: cfg.aws_access_key_id,
        secretAccessKey: cfg.aws_secret_access_key
    });
    return new Promise((ok, nok) => {
        let evalResult = msg.body;
        debug('Calling lambda function=%s region=%s', cfg.functionName, cfg.region);
        const expression = cfg.expression;
        if (expression) {
            debug('Evaluating expression="%s"', expression);
            const compiledExpression = jsonata(expression);
            evalResult = compiledExpression.evaluate(msg.body);
            debug('Evaluation completed, result=%j', evalResult);
        }
        lambda.invoke({
            FunctionName: cfg.functionName,
            InvocationType: cfg.invocationType || 'RequestResponse',
            LogType: cfg.logType || 'None',
            Payload: JSON.stringify(evalResult),
            Qualifier: cfg.Qualifier || '$LATEST'
        }, (err, result) => {
            if (err) {
                debug('Failed to invoike function=%s error=%s', cfg.functionName, err.stack || err);
                return nok(err);
            }
            debug('Successfull response: %j', result);
            if (result.LogResult) {
                console.log('LogResult: ', new Buffer(result.LogResult, 'base64').toString('utf8'));
                delete result.LogResult;
            }
            try {
                if (result.Payload) {
                    const json = JSON.parse(result.Payload);
                    ok(messages.newMessageWithBody(json));
                }
            } catch (err) {
                // Ignore it now
            }
            debug('Resulting payload is not JSON, sending it as string');
            return ok(messages.newMessageWithBody(result));
        });
    });
}

module.exports.process = processAction;
